...
function searchAjax(up,type,mainHeadSearch){
	...
	if(type=="searchall"){
		url="servlet/SalaryServlet?para=searchstaff&searchvalue=&up="+up;
	}else if(type=="searchsingle"){
		if(mainHeadSearch==""){
			alert("请输入关键字！");
			return;
		}
		url="servlet/SalaryServlet?para=searchstaff&searchvalue="+mainHeadSearch+"&up="+up;
	}	
...
//	alert(Object.keys(jsonObj[0])[7]);
	...
	for(var i=0;i<row+1;i++){
		...
		tr.onclick = function (){
			...
			var trsalary = this.getElementsByTagName("td")[3].innerHTML;
...
function changeSt(){
	...
	//获取当前时间
	var lastdate = getNowFormatDate();

	changeAjax(staffid,lastdate);
}


function changeAjax(staffid,lastdate){
	
	var xmlHttp = initAjax();
	xmlHttp.open("get","servlet/SalaryServlet?para=changeSt&staffid="+staffid+"&lastdate="+lastdate,true);
	xmlHttp.send(null);	
	xmlHttp.onreadystatechange = function ()
	{
		if (xmlHttp.readyState == 4) {
            if (xmlHttp.status == 200) { 
            	...
            	if(tip=="ok"){
            		alert("计算完成！");
            		searchAjax(0,"searchsingle",staffid);
            	}else{
            		alert("失败！");
            	}
