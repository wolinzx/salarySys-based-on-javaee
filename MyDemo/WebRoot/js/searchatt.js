...
function searchAjax(up,type,mainHeadSearch){
	...
	if(type=="searchall"){
		url="servlet/AttServlet?para=searchstaff&searchvalue=&up="+up;
	}else if(type=="searchsingle"){
		if(mainHeadSearch==""){
			alert("请输入关键字！");
			return;
		}
		url="servlet/AttServlet?para=searchstaff&searchvalue="+mainHeadSearch+"&up="+up;
	}	
...
function changeAjax(att,staffid){
	...
	var xmlHttp = initAjax();
	xmlHttp.open("get","servlet/AttServlet?para=changeSt&att="+att+"&staffid="+staffid,true);
	xmlHttp.send(null);	
	xmlHttp.onreadystatechange = function ()
	{
		if (xmlHttp.readyState == 4) {
